...
def main(args):
    ...
    for t in tqdm(t_values):
        ...
        point_metric_dict, image_metric_dict = evaluator.evaluate_person_random_proj()
        for cat in cat_list:
            per_image_pck[cat].append(image_metric_dict[cat])
            per_point_pck[cat].append(point_metric_dict[cat])

    with open(os.path.join(args.plot_path, f'{cat}_evaluation_curve_Per_Image_PCK@0.1.txt'), 'w') as f:
        for tt, ipck in zip(t_values, per_image_pck[cat]):
            f.write(str(tt) + '\t' + str(ipck) + '\n')
    
    with open(os.path.join(args.plot_path, f'{cat}_evaluation_curve_Per_Point_PCK@0.1.txt'), 'w') as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_path', type=str, default='/scratch/lt453/spair_ft_person/', help='path to save features')
    ...
    parser.add_argument('--plot_path', type=str, default='./plots_person_random_proj', help='path to save plots')
