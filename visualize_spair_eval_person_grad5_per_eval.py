...
def main(args):
    ...
    feature_folder = './plot_res/plots_person_grad5'
        ...
        for t in t_values:
            ...
            for cat in ['person']:
                ...
                for json_path in tqdm(evaluator.cat2json[cat]):
                    ...
                    src_z_grad = src_z_grad.reshape(1, 1, 6, 12)
                    up_src_z_grad = nn.Upsample(size=(48, 48), mode='bilinear')(src_z_grad)
                    up_src_z_grad = up_src_z_grad[0,...]
                    up_src_z_grad = up_src_z_grad.repeat(1280, 1, 1)
                    up_src_z_grad = -0.5*betas[t] * up_src_z_grad
                    # print(f'src norms {torch.norm(src_fet)}, {torch.norm(src_t_grad)}, {torch.norm(up_src_z_grad)}')
                    ...
                    trg_t_grad = trg_t_grad[:, :, :, 0]
                    trg_z_grad = torch.from_numpy(np.load(os.path.join(args.plot_path, str(t), trg_imname +'_z_gradient.npy'))).cuda()
                    trg_z_grad = trg_z_grad.reshape(1, 1, 6, 12)
                    up_trg_z_grad = nn.Upsample(size=(48, 48), mode='bilinear')(trg_z_grad)
                    up_trg_z_grad = up_trg_z_grad[0,...]
                    up_trg_z_grad = up_trg_z_grad.repeat(1280, 1, 1)
                    up_trg_z_grad = -0.5*betas[t] * up_trg_z_grad
                    # print(f'trg norms {torch.norm(trg_fet)}, {torch.norm(trg_t_grad)}, {torch.norm(up_trg_z_grad)}')
                    ...
                    for idx in range(len(data['src_kps'])):
                        ...
                        src_fet = src_fet + w1*src_t_grad + w2*up_src_z_grad
                        resized_src_fet = resize(src_fet, src_img_size)
                            
                        trg_fet = trg_fet + w1*trg_t_grad + w2*up_trg_z_grad
    ...
    plt.legend()
    plt.savefig(os.path.join(args.save_path, f'Person_Per_Image_evaluation_curve_tzbetagrads2.png'))
    ...
    plt.title(f'PCK Metric vs. t - Category: Person')
    plt.legend()
    plt.savefig(os.path.join(args.save_path, f'Person_Per_Point_evaluation_curve_tzbetagrads2.png'))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_path', type=str, default='./plot_res/plots_person_grad5_eval', help='path to save features')
    ...
    parser.add_argument('--plot_path', type=str, default='./plot_res/plots_person_grad5_per_eval', help='path to save plots')
