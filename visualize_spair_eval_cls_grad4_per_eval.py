...
def main(args):
    ...
    feature_folder = f'./plot_res/plots_{args.cls_name}_grad4'
    ...
    wss = [ws1, ws2, ws3]
    for ws_idx, ws in enumerate(wss):
        ws_idx += 4
        per_point_pck_ress = []
        per_image_pck_ress = []
        for w0, w1, w2, w3, w4, w5, w6, w7, w8, w9, w10, w11 in ws:
            values1 = []
            values2 = []
            per_point_pck_res = []
            per_image_pck_res = []
            for t in t_values:
                ...
                for cat in [args.cls_name]:
                    ...
                    for json_path in tqdm(evaluator.cat2json[cat]):
                        ...
                        src_gra = torch.load(os.path.join(feature_folder, str(t), src_imname +'_gradient.pth')).cuda()
                        # print(f'src gradient: {torch.norm(src_fet)}, {torch.norm(src_gra)}')
                        # src_gra = resize(src_gra, src_img_size)
                        src_gra_t_10 = torch.load(os.path.join(feature_folder, str(min(t+10, max(t_values))), src_imname +'_gradient.pth')).cuda()
                        # src_gra_t_10 = resize(src_gra_t_10, src_img_size)
                        src_gra_t_20 = torch.load(os.path.join(feature_folder, str(min(t+20, max(t_values))), src_imname +'_gradient.pth')).cuda()
                        # src_gra_t__10 = resize(src_gra_t__10, src_img_size)
                        src_gra_t_30 = torch.load(os.path.join(feature_folder, str(min(t+30, max(t_values))), src_imname +'_gradient.pth')).cuda()
                        # src_gra_t__20 = resize(src_gra_t__20, src_img_size)
                        src_gra_t_40 = torch.load(os.path.join(feature_folder, str(min(t+40, max(t_values))), src_imname +'_gradient.pth')).cuda()
                        src_gra_t_50 = torch.load(os.path.join(feature_folder, str(min(t+50, max(t_values))), src_imname +'_gradient.pth')).cuda()
                        src_gra_t_60 = torch.load(os.path.join(feature_folder, str(min(t+60, max(t_values))), src_imname +'_gradient.pth')).cuda()
                        src_gra_t_70 = torch.load(os.path.join(feature_folder, str(min(t+70, max(t_values))), src_imname +'_gradient.pth')).cuda()
                        src_gra_t_80 = torch.load(os.path.join(feature_folder, str(min(t+80, max(t_values))), src_imname +'_gradient.pth')).cuda()
                        src_gra_t_90 = torch.load(os.path.join(feature_folder, str(min(t+90, max(t_values))), src_imname +'_gradient.pth')).cuda()
                        src_gra_t_100 = torch.load(os.path.join(feature_folder, str(min(t+100, max(t_values))), src_imname +'_gradient.pth')).cuda()
                        ...
                        trg_gra = torch.load(os.path.join(feature_folder, str(t), trg_imname +'_gradient.pth')).cuda()
                        # trg_gra = resize(trg_gra, trg_img_size)
                        trg_gra_t_10 = torch.load(os.path.join(feature_folder, str(min(t+10, max(t_values))), trg_imname +'_gradient.pth')).cuda()
                        # trg_gra_t_10 = resize(trg_gra_t_10, trg_img_size)
                        trg_gra_t_20 = torch.load(os.path.join(feature_folder, str(min(t+10, max(t_values))), trg_imname +'_gradient.pth')).cuda()
                        # trg_gra_t__10 = resize(trg_gra_t__10, trg_img_size)
                        trg_gra_t_30 = torch.load(os.path.join(feature_folder, str(min(t+30, max(t_values))), trg_imname +'_gradient.pth')).cuda()
                        trg_gra_t_40 = torch.load(os.path.join(feature_folder, str(min(t+40, max(t_values))), trg_imname +'_gradient.pth')).cuda()
                        trg_gra_t_50 = torch.load(os.path.join(feature_folder, str(min(t+50, max(t_values))), trg_imname +'_gradient.pth')).cuda()
                        trg_gra_t_60 = torch.load(os.path.join(feature_folder, str(min(t+60, max(t_values))), trg_imname +'_gradient.pth')).cuda()
                        trg_gra_t_70 = torch.load(os.path.join(feature_folder, str(min(t+70, max(t_values))), trg_imname +'_gradient.pth')).cuda()
                        trg_gra_t_80 = torch.load(os.path.join(feature_folder, str(min(t+80, max(t_values))), trg_imname +'_gradient.pth')).cuda()
                        trg_gra_t_90 = torch.load(os.path.join(feature_folder, str(min(t+90, max(t_values))), trg_imname +'_gradient.pth')).cuda()
                        trg_gra_t_100 = torch.load(os.path.join(feature_folder, str(min(t+100, max(t_values))), trg_imname +'_gradient.pth')).cuda()
                        ...
                        for idx in range(len(data['src_kps'])):
                            ...
                            src_fet = w0 * src_fet + w1*src_gra + w2*src_gra_t_10 + w3*src_gra_t_20 + w4*src_gra_t_30 + w5*src_gra_t_40 + w6*src_gra_t_50 + w7*src_gra_t_60 + w8*src_gra_t_70 + w9*src_gra_t_80 + w10*src_gra_t_90 + w11*src_gra_t_100
                            resized_src_fet = resize(src_fet, src_img_size)
                                
                            trg_fet = w0 * trg_fet + w1*trg_gra + w2*trg_gra_t_10 + w3*trg_gra_t_20 + w4*trg_gra_t_30 + w5*trg_gra_t_40 + w6*trg_gra_t_50 + w7*trg_gra_t_60 + w8*trg_gra_t_70 + w9*trg_gra_t_80 + w10*trg_gra_t_90 + w11*trg_gra_t_100
        ...
        for res, ww in zip(per_image_pck_ress, ws):
            ww0, ww1, ww2, ww3, ww4, ww5, ww6, ww7, ww8, ww9, ww10, ww11 = ww
            wsstr = str(ww0) + '_' + str(ww1) + '_' + str(ww2) + '_' + str(ww3) + '_' + str(ww4) + '_' + str(ww5) + '_' + str(ww6) + '_' + str(ww7) + '_' + str(ww8) + '_' + str(ww9) + '_' + str(ww10) + '_' + str(ww11)
            plt.plot(t_values, res, label=f'{wsstr}')
        plt.xlabel('t')
        plt.ylabel('PCK')
        plt.title(f'PCK Metric vs. t - Category: {args.cls_name}')
        plt.legend()
        plt.savefig(os.path.join(args.save_path, f'{args.cls_name}_Per_Image_evaluation_curve_{ws_idx}.png'))
        ...
        plt.title(f'PCK Metric vs. t - Category: {args.cls_name}')
        plt.legend()
        plt.savefig(os.path.join(args.save_path, f'{args.cls_name}_Per_Point_evaluation_curve_{ws_idx}.png'))
