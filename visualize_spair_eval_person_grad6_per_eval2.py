...
def main(args):
    ...
    ws = [[0, 0], [10, 0], [10, 3], [10, 5], [10, 10], [10, 100]]
    per_point_pck_ress = []
    per_image_pck_ress = []
    for w1, w2 in ws:
        values1 = []
        ...
        for t in t_values:
            ...
            for cat in ['person']:
                ...
                for json_path in tqdm(evaluator.cat2json[cat]):
                    ...
                    print(src_z_grad.shape)
                    # print(torch.all(src_z_grad[0, :, :, :] == src_z_grad[1, :, :, :]))
                    src_z_grad = -0.5*betas[t] * src_z_grad
                    # print(src_z_grad[:2, :2, :2, :2])
                    print(src_z_grad[:4, 0, 0])
                    print(src_z_grad[0, :4, 0])
                    print(src_z_grad[0, 0, :4])
                    # print(src_z_grad[:4, 0, 0, 0])
                    # print(src_z_grad[0, :4, 0, 0])
                    # print(src_z_grad[0, 0, :4, 0])
                    # print(src_z_grad[0, 0, 0, :4])
    ...
    plt.legend()
    plt.savefig(os.path.join(args.save_path, f'Person_Per_Image_evaluation_curve_{args.repeat_direc}_nobv2.png'))
    ...
    plt.title(f'PCK Metric vs. t - Category: Person')
    plt.legend()
    plt.savefig(os.path.join(args.save_path, f'Person_Per_Point_evaluation_curve_{args.repeat_direc}_nobv2.png'))
