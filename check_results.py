...
for file in files:
    ...
    for idx in range(len(data['src_kps'])):
        ...
        # if not os.path.exists(os.path.join(npy_dir, src_feature_path)) or not os.path.exists(os.path.join(npy_dir, src_gradient_path)) or not os.path.exists(os.path.join(npy_dir, trg_feature_path)) or not os.path.exists(os.path.join(npy_dir, trg_gradient_path)):
        #     continue
        
        # src_feat = torch.from_numpy(np.load(os.path.join(npy_dir, src_feature_path)).reshape(1, 1280)).cuda()
        # src_feat_norm = F.normalize(src_feat, dim=1)
        # trg_feat = torch.from_numpy(np.load(os.path.join(npy_dir, trg_feature_path)).reshape(1280, 1)).cuda()
        # trg_feat_norm = F.normalize(trg_feat, dim=0)
        
        # src_grad = torch.from_numpy(np.load(os.path.join(npy_dir, src_gradient_path)).reshape(1, 1280)).cuda()
        # src_grad += src_feat
        # src_grad_norm = F.normalize(src_grad, dim=1)
        # trg_grad = torch.from_numpy(np.load(os.path.join(npy_dir, trg_gradient_path)).reshape(1280, 1)).cuda()
        # trg_grad += trg_feat
        # trg_grad_norm = F.normalize(trg_grad, dim=0)
        
        # cos_feat = torch.mm(src_feat_norm, trg_feat_norm)
        # cos_feat = cos_feat.cpu().numpy()[0, 0]
        
        # cos_grad = torch.mm(src_grad_norm, trg_grad_norm)
        # cos_grad = cos_grad.cpu().numpy()[0, 0]
        
        # xx += 1
        # # print(cos_feat, cos_grad)
