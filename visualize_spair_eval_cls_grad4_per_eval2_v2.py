...
def main(args):
    ...
    for ws_idxx, ws in enumerate(wss):
            ...
            for t in t_values:
                ...
                for cat in [args.cls_name]:
                    ...
                    for json_path in tqdm(evaluator.cat2json[cat]):
                        ...
                        if w_idx != 0:
                            vv = 10*sum(src_grads[max(t//10-w_idx, 0):min(t//10+w_idx, len(src_grads))])                        
                            if args.neg_direc:
                                src_fet -= vv    
                            else:
                                src_fet += vv
                        ...
                        if w_idx != 0:
                            yy = 10*sum(trg_grads[max(t//10-w_idx, 0):min(t//10+w_idx, len(trg_grads))]) 
                            if args.neg_direc:
                                trg_fet -= yy
                            else:
                                trg_fet += yy
        ...
        if args.neg_direc and args.neg_vec:
            plt.savefig(os.path.join(args.save_path, f'{args.cls_name}_Per_Image_evaluation_curve_restall_{ws_idxx}_negrec_negvecv2.png'))    
        elif args.neg_vec:
            plt.savefig(os.path.join(args.save_path, f'{args.cls_name}_Per_Image_evaluation_curve_restall_{ws_idxx}_negvecv2.png'))    
        elif args.neg_direc:
            plt.savefig(os.path.join(args.save_path, f'{args.cls_name}_Per_Image_evaluation_curve_restall_{ws_idxx}_negrecv2.png'))    
        else:
            plt.savefig(os.path.join(args.save_path, f'{args.cls_name}_Per_Image_evaluation_curve_restall_{ws_idxx}v2.png'))
        ...
        plt.legend()
        if args.neg_direc and args.neg_vec:
            plt.savefig(os.path.join(args.save_path, f'{args.cls_name}_Per_Point_evaluation_curve_restall_{ws_idxx}_negrec_negvecv2.png'))    
        elif args.neg_vec:
            plt.savefig(os.path.join(args.save_path, f'{args.cls_name}_Per_Point_evaluation_curve_restall_{ws_idxx}_negvecv2.png'))    
        elif args.neg_direc:
            plt.savefig(os.path.join(args.save_path, f'{args.cls_name}_Per_Point_evaluation_curve_restall_{ws_idxx}_negrecv2.png'))    
        else:
            plt.savefig(os.path.join(args.save_path, f'{args.cls_name}_Per_Point_evaluation_curve_restall_{ws_idxx}v2.png'))
