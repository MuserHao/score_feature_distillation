...
def main(args):
    ...
    for cls_name in run_names:
        ...
        for ws_idxx, ws in enumerate(wss):
            ...
            for w_idx in ws:
                ...
                for t in t_values:
                    ...
                    for json_path in tqdm(evaluator.cat2json[cat]):
                        ...
                        if w_idx == -1:
                            src_fet += 10*sum(src_grads[t//10:])
                        elif w_idx != 0:
                            xx = src_grads[t//10:t//10+w_idx]
                            if args.neg_direc:
                                src_fet -= 10*sum(xx)    
                            else:
                                src_fet += 10*sum(xx)
                        ...
                        if w_idx == -1:
                            trg_fet += 10*sum(trg_grads[t//10:])
                        elif w_idx != 0:
                            yy = trg_grads[t//10:t//10+w_idx]
                            if args.neg_direc:
                                trg_fet -= 10*sum(yy)    
                            else:
                                trg_fet += 10*sum(yy)
                    ...
                    dicts_image_pck[t] = per_image_pck_dict[args.cls_name]
                    dicts_point_pck[t] = per_point_pck_dict[args.cls_name]
                
                fol = os.path.join(args.save_path, 'jsons')
                os.makedirs(fol, exist_ok=True)
                with open(os.path.join(args.save_path, 'jsons', f'{args.cls_name}_Per_Image_evaluation_curve_acc_{w_idx}.json'), 'w') as f:
                    json.dump(dicts_image_pck, f)
                
                with open(os.path.join(args.save_path, 'jsons', f'{args.cls_name}_Per_Point_evaluation_curve_acc_{w_idx}.json'), 'w') as f:
            ...
            plt.legend()
            if args.neg_direc:
                plt.savefig(os.path.join(args.save_path, f'{args.cls_name}_Per_Image_evaluation_curve_acc_{ws_idxx}_negrec.png'))    
            else:
                plt.savefig(os.path.join(args.save_path, f'{args.cls_name}_Per_Image_evaluation_curve_acc_{ws_idxx}.png'))
            ...
            plt.legend()
            if args.neg_direc:
                plt.savefig(os.path.join(args.save_path, f'{args.cls_name}_Per_Point_evaluation_curve_acc_{ws_idxx}_negrec.png'))    
            else:
                plt.savefig(os.path.join(args.save_path, f'{args.cls_name}_Per_Point_evaluation_curve_acc_{ws_idxx}.png'))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cls_names', type=str, default='cls_names1')
