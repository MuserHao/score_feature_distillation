...
class SPairEvaluator:
    ...
    def evaluate_person_grad4(self, cls_name=['person'], path=None):
        torch.cuda.set_device(0)
        torch.set_grad_enabled(True)

        res = defaultdict(list)
        for cat in tqdm(cls_name):
            if path != None:
                self.plot_path = path + f'/plots_{cat}_grad4'
    ...
    def evaluate_person_grad4tz(self, cls_name=['person'], path=None, t_values = range(0, 999, 10)):
        torch.cuda.set_device(0)
        torch.set_grad_enabled(True)
        ...
        for cat in tqdm(cls_name):
            for t in tqdm(t_values):
                ...
                print(cat, self.t)
                if path != None:
                    self.plot_path = path + f'/plots_{cat}_grad4tz'
                ...
                for image_path, points in tqdm(dicts.items()):
                    ...
                    import time
                    time1 = time.time()
                    feature, gradients_t, gradients_z = self.dift.forward_grad4tz(
                                                img,
                                                category=cat,
                                                img_size=self.img_size,
                                                t=self.t,
                                                ...
                                                image_size=points[0],
                                                points = None)
                    # print(feature.shape, gradients_t.shape, gradients_z.shape)
                    # print(feature.shape, gradients.shape, time.time() - time1)
                    pre_folder = os.path.join(self.plot_path, str(self.t))
                    os.makedirs(pre_folder, exist_ok=True)
                    save_feature = os.path.join(self.plot_path,str(self.t), image_path + '_feature.pth')
                    ...
                    torch.save(gradients_z, save_gradient)
                    # with open(os.path.join(self.plot_path, str(self.t), image_path + '_bbox.json'), 'w') as f:
                    #     json.dump(points, f)
